// 地図初期化
const map = L.map('map').setView([33.2635, 130.3009], 9);
...
const markerClusterGroup = L.markerClusterGroup({
    maxClusterRadius: 40 // ← 半径を小さくして緩める
...
function getCenterOfWork(workKey) {
    const spots = spotData[workKey];
    if (!spots || spots.length === 0) return null;
    const latSum = spots.reduce((sum, s) => sum + s.lat, 0);
    const lngSum = spots.reduce((sum, s) => sum + s.lng, 0);
    return [latSum / spots.length, lngSum / spots.length];
}

function getSpotIcon(spot, isVisited) {
    return L.icon({
        iconUrl: spot.iconUrl || "images/default.png",
        iconSize: [60, 60],
        iconAnchor: [30, 60],
...
function loadSpots() {
    ...
    Object.keys(spotData).forEach(workKey => {
        const visited = JSON.parse(localStorage.getItem(getStorageKey(workKey)) || "{}");
        visitedAll[workKey] = visited;
    });

    // ✅ クラスタをクリア
    markerClusterGroup.clearLayers();

    const worksToShow = work ? [work] : Object.keys(spotData);

    worksToShow.forEach(workKey => {
        spotData[workKey].forEach(spot => {
            const isVisited = visitedAll[workKey][spot.id];
            const marker = L.marker([spot.lat, spot.lng], {
                icon: getSpotIcon(spot, isVisited)
            });

            marker.on("click", () => {
                const content = document.getElementById("infoContent");
                const imageClass = isVisited ? "visited" : "unvisited";
                const buttonLabel = isVisited ? "スタンプ解除" : "スタンプ押印";

                const userLatLng = userLocationMarker?.getLatLng();
                const googleMapUrl = userLatLng
                    ? `https://www.google.com/maps/dir/?api=1&origin=${userLatLng.lat},${userLatLng.lng}&destination=${spot.lat},${spot.lng}&travelmode=walking`
                    : `https://www.google.com/maps/search/?api=1&query=${spot.lat},${spot.lng}`;

                content.innerHTML = `
                    <img src="${spot.iconUrl}" alt="${spot.name}" class="spot-image ${imageClass}" id="spotImage"><br>
                    <h3>${spot.name}</h3>
                    <p>営業時間：${spot.hours || "不明"}</p>
                    <p>休みの日：${spot.closedDays || "不明"}</p>
                    <button onclick="stampSpot('${spot.id}', '${spot.name}', '${workKey}', '${spot.iconUrl}')">
                      ${buttonLabel}
                    </button>
                    <button onclick="startNavigation(${spot.lat}, ${spot.lng}); document.getElementById('infoOverlay').style.display = 'none';">
                      ナビ開始
                    </button>
    ...
    if (work && !hasCentered) {
        const center = getCenterOfWork(work);
        if (center) {
...
function stampSpot(id, name, workKey, imageUrl) {
    const key = `visitedSpots_${workKey}`;
    ...
    if (!isVisited) {
        fetch("https://your-stamp-api.example.com/save", {
            method: "POST",
